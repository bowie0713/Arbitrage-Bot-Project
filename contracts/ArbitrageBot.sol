...
contract Arbitrage{
    ...
    ISwapRouter public immutable swapRouter = ISwapRouter(SWAP_ROUTER_02); 
    ICurvePool private constant curvepoolAddress = ICurvePool(0x7F86Bf177Dd4F3494b841a37e810A34dD56c829B);

    function depositWETH() external payable{
        WETH.deposit{value: msg.value}();
        // WETH.transfer(msg.sender, WETH.balanceOf(address(this)));
        // no need to transfer again. Also the deposit has not been finalized
    }
    ...
    function getTokenAmountOnUniswap(uint256 amountIn) external view returns(uint256 amountOut){
        IUniswapV3Factory uniswapFactory = IUniswapV3Factory(0x1F98431c8aD98523631AE4a59f267346ea31F984); // Uniswap V3 factory address
        address addressPool = uniswapFactory.getPool(address(USDC), address(WETH), 1000);
        ...
        uint256 priceOut = (uint256(sqrtPriceX96)^2) / (1 << 192); // (1 << 192) refers to shifting 1 by 192 bits = 1*(2^(192))
        amountOut = priceOut * amountIn;
    } 
    function buyETHonUniswap(uint256 amountIn) external returns(uint256 amountOut){
        require(IERC20(USDC).balanceOf(address(this)) >= amountIn, "USDC Balance Not Enough");
    ...
    function buyEthonCurve(uint256 usdcAmountIn, uint256 minEthAmount) public returns (uint256 amountOut){
        // Swap USDC for ETH, assuming USDC is 0 and ETH is 1
        require(IERC20(USDC).balanceOf(address(this)) >= usdcAmountIn, "USDC Balance Not Enough");
        IERC20(address(USDC)).approve(address(curvepoolAddress), usdcAmountIn);
        amountOut = curvepoolAddress.exchange(0, 2, usdcAmountIn, minEthAmount);
    }
    function sellEthonCurve(uint256 ethAmountIn, uint256 minUsdcAmount) public returns (uint256 amountOut){
        require(WETH.balanceOf(address(this)) >= ethAmountIn, "WETH Balance Not Enough");
        IERC20(address(WETH)).approve(address(curvepoolAddress), minUsdcAmount);
        amountOut = curvepoolAddress.exchange(2, 0, ethAmountIn, minUsdcAmount);
    }
    function getTokenAmountonCurve(uint256 usdcamount) public view returns (uint256){
        return curvepoolAddress.get_dy(0, 2, uint128(usdcamount)); // same as above
   }
